...
class AdminPanelBooksList extends Component {
  ...
  render() {
    return (
      let item;
    if (this.props.book) {
      item = this.state.book.map((book, index) => {
        return (
          item = <div className="Admin__book-item row">
            <div className="col-md-2 Admin__img-box">
              <center><img src={book.image} alt={this.props.book.name} className="book-img" /></center>
            </div>
          </div>
        );
      });
